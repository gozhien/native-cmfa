...
class TunService : VpnService(), CoroutineScope by CoroutineScope(Dispatchers.Default) {
    ...
    private fun startZivpnCores() {
        ...
        val receiveWindow = zivpnStore.receiveWindow
        val receiveWindowConn = zivpnStore.receiveWindowConn
        ...
        try {
            ...
            for (i in 0 until 4) {
                ...
                val configContent = """{"server":"$serverHost:$range","obfs":"$obfs","auth":"$pass","socks5":{"listen":"127.0.0.1:$port"},"insecure":true,"recvwindowconn":$receiveWindowConn,"recvwindow":$receiveWindow,"up_mbps":$up,"down_mbps":$down}"""
